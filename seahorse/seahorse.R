...
count_gd = function(d,ln,l=0){
  ...
  # l = *BROKEN DO NOT USE* defaulting to 0 --WILL FIX LATER--
  if (ln > max(d)){
    return(0)
  }
  d = fix_series(d) #setting minimum at ends of x
  d[d < ln] = ln #this sets values smaller than it as the height
                 #to make it easier to use gridDiag
  tmp = gridDiag(FUNvalues = d, sublevel = FALSE)[['diagram']] #this returns the births/deaths
  bd = tmp[,3] - tmp[,2]
  tmp = tmp[,2]
...
#
######################################################################
count_gd_heights = function(d,day,lts,l=0,omax=F,nheight=100){
  ...
  day_max = max(d[,,day]) #max of heights to scan for
  day_min = min(d[,,day]) #min of heights to scan for
  if (omax == T){ #used for making bases in build_comp below
    day_max = 6018
    day_min = 4594
  }
  fts = NULL
  hts = seq(day_min,day_max,length.out = nheight) #vector of heights to scan
  # hts is which heights to check
  for (i in hts){
    fts = c(fts,count_gd(d[,lts,day],i,l=0)) #runs through all the heights
  }
  return(fts) #returns a vector of detected features for a particular latitude
}
...
#
######################################################################
count_gd_day = function(d,day,plot=F,title="",l=0,summary=F,omax=F,nheight=100){
  ...
  for (i in 1:n){ #this loop creates all the latitudes
    mtx = cbind(mtx,count_gd_heights(d,day,i,l=0,omax=omax,nheight=nheight))
    if (summary == T){
  ...
  if (plot == T){ #plots the information
    ...
    if (summary == T){ #plots mean, median, quartiles, etc...
  ...
  return(mtx) #returns only a matrix of information if plot=F
}
